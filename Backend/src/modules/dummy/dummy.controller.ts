...
import {
	FilterQueryParams,
	getMappedPrismaFilterArgs,
	getMappedPrismaOrderByArgs,
	OrderByQueryParams,
...
export class DummyController {
	...
	getWithObjectUrlParams(@Query() someObj: SomeObjDto) {
		return someObj;
	}

	@Get("getWithArrayParams")
	getWithArrayParams(
		@Query("excludeIds", new ParseArrayPipe({ items: Number, optional: true }))
		excludeIds: number[] = []
	) {
		return excludeIds;
	}
	...
	@Get("test")
	async getTestEntities(@Query() query: InfiniteLoaderQueryDto) {
		const { prisma } = this;
	...
	@Get("test/pager")
	async getTestEntitiesWithPager(@Query() query: PagerQueryDto) {
		const { prisma } = this;
