    ...
    protected create(
        ...
        return async (
            ...
            if (params.model === entityName && (events as string[]).includes(params.action)) {
                ...
                if (options?.onlyIf) {
                    const [shouldPass, ctx] = await options.onlyIf(
                        params.action as Event,
                        params.args
                    );
                ...
                const title =
                    (options?.titleEvaluator &&
                        (await options.titleEvaluator(event, params.args, context))) ||
                    `${entityName} ${TicketActivityEventLabelMap[event]}`;

                const description =
                    options?.descriptionEvaluator &&
                ...
                const createdTicketActivity = await this.prisma.ticketActivity.create({
                    data: {
                        ticketId:
                            entity.ticketId ||
                            (options?.ticketIdEvaluator &&
                                (await options.ticketIdEvaluator(event, params.args, context))),
                        title,
                ...
                if (!options?.disableMailDelivery) {
                    this.ticketActivityMailProvider.sendEmailToWatchingUsers(
                        createdTicketActivity.id
                    );
                }
            }
