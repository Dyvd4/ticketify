...
import {
    BaseTicketActivityPrismaMiddlewareProvider,
    TicketActivityEventColorMap,
    TicketActivityEventLabelMap,
    TicketActivityModelIconMap,
} from "./base-ticket-activity-prisma-middleware.provider";
...
describe("TicketActivityPrismaMiddlewareProvider", () => {
    ...
    beforeEach(async () => {
        ...
        ticketActivityPrismaMiddlewareProvider = moduleRef.get(
            TestTicketActivityPrismaMiddlewareProvider
        );
    });
    ...
    describe("optional parameters", () => {
        it("allows async evaluators", async () => {
            const CreateTicketActivityBasedOnTestFn = ticketActivityPrismaMiddlewareProvider.create(
                "Comment",
                ["create", "update"],
                {
            ...
            expect(ticketActivityCreateFn).toHaveBeenNthCalledWith(
                1,
        ...
        describe("events", () => {
            it("allows to filter out events", async () => {
                const CreateTicketActivityBasedOnTestFn =
                    ticketActivityPrismaMiddlewareProvider.create("Comment", ["update"], {
                        disableMailDelivery: true,
                        descriptionEvaluator: async (event) => {
                            if (event === "update")
                                return commentUpdatedActivityDescription; // should be
    ...
    describe("when creating activity", () => {
        const CreateTicketActivityBasedOnTrigger = () => {
            ticketActivityPrismaMiddlewareProvider.create("Comment", ["create", "update"], {
                disableMailDelivery: true,
            })(
                {
                    model: "Comment",
