...
export class AuthController {
    ...
    @Post("signIn")
    async signIn(@Body() { username, password }: UserSignInDto, @Res() response: Response) {
        const { prisma } = this;
    ...
    @Post("signUp")
    async signUp(@Body() { username, password, email }: UserSignUpDto, @Res() response: Response) {
        const { prisma } = this;
    ...
    @Post("confirmEmail")
    async createConfirmationEmail(@User() requestUser: TUser) {
        const { prisma } = this;
    ...
    async confirmConfirmationEmail(
        ...
        const {
            data: { userId },
        } = jwt.verify(redirectToken, JWT_SECRET_KEY) as { data: { userId: string } };
