	...
	const mappedFilter = prismaFilter
		...
		.reduce((map, filter) => {
			if (filter.value !== null && filter.value !== "") {
				Object.set(
					map,
					`AND.${filter.property}.${filter.operation?.value}`,
					getParsedFilterValue(filter.value, filter.type)
				);
			}
			return map;
		}, {});
...
export const getMappedPrismaOrderByArgs = (
	prismaOrderBy: OrderByQueryParams | OrderByQueryParam
) => {
	...
	if ("length" in prismaOrderBy) {
		return prismaOrderBy.filter((orderBy) => !orderBy.disabled).map(mappedOrderBy);
	}
