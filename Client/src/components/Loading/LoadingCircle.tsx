...
function LoadingCircle(props: Props) {
    ...
    let className = (props.className) ? props.className : "absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50";
    ...
    else if (useCover && !loading && loadingCoverRef.current && loadingCoverRef.current.parentElement) {
        let parent = loadingCoverRef.current.parentElement;
    ...
    if (usePortal) {
        return ReactDom.createPortal(
            <div className={className}>
                <div style={{ width: size, height: size, borderWidth }}
