...
function LoadingBorder({ className, animationDurationMs, ...props }: LoadingBorderProps) {
    ...
    useEffect(() => {
        if (animationDurationMs) {
            document.documentElement.style.setProperty("--loading-border-animation-durationMs", animationDurationMs + "ms")
        }
    }, []);
    ...
    return (
        <div
            id="test"
            className={`relative loading-border overflow-hidden ${className}`}
            {...props}>
            <div className="w-full h-1
                            bg-gradient-to-r from-transparent to-cyan-500"></div>
            <div className="h-full w-1
                            bg-gradient-to-b from-transparent to-cyan-500"></div>
            <div className="w-full h-1
                            bg-gradient-to-l from-transparent to-cyan-500"></div>
            <div className="h-full w-1
                            bg-gradient-to-t from-transparent to-cyan-500"></div>
            {props.children}
        </div>
    );
}
