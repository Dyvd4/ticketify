...
const testKanbanGroups: KanbanboardProps<{ id: number, title: string, description: string }>["groups"] = [
    {
        name: "Category A",
        items: [
...
test("orders items", () => {
    ...
    expect(within(groupCListItems[0]).getByText(testKanbanGroups[2].items[1].description)).toBeTruthy();
    expect(within(groupCListItems[1]).getByText(testKanbanGroups[2].items[0].description)).toBeTruthy();
    expect(within(groupCListItems[2]).getByText(testKanbanGroups[2].items[3].description)).toBeTruthy();
    expect(within(groupCListItems[3]).getByText(testKanbanGroups[2].items[2].description)).toBeTruthy();
});