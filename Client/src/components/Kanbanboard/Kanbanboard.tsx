...
function Kanbanboard<T extends { id: any }>({
	...
	// state
	// -----
	const [recentDraggedOverGroupName, setRecentDraggedOverGroupName] = useState<
		string | undefined
	>();
	...
	const getSortedGroups = (
		groups: _KanbanboardProps<T>["groups"]
	): _KanbanboardProps<T>["groups"] => {
	...
	const handleOnDrop = async (e: React.DragEvent<HTMLLIElement>, groupName: string) => {
		...
		for (let i = 0; i < groupsCopy.length; i++) {
			const itemIndex = groupsCopy[i].items.findIndex(
				(item) => item.id === droppedGroupItem.id
			);
			if (itemIndex !== -1) {
		...
		if (droppedGroupItemGroupIndex !== undefined && droppedGroupItemIndex !== undefined) {
			...
			groupsCopy[groupsCopy.findIndex((group) => group.name === groupName)].items.push(
				droppedGroupItem
			);
	...
	return !isLoading ? (
		<ul className={`grid list-none grid-cols-3 gap-4 ${className}`} {...props}>
			{groups.map((group) => (
				<li
					...
					key={group.name}
					className={"transform rounded-md border-cyan-500 transition-all"}
				>
					<BgBox className="rounded-b-none border-b text-center">{group.name}</BgBox>
					<BgBox
						className={classNames("relative rounded-t-none", {
							"border-2 border-dashed border-cyan-500":
								recentDraggedOverGroupName === group.name && isDragging,
						})}
					>
						...
						>
							{group.items.map((item) => (
								...
								>
									<BgBox variant="child">{groupItemsRenderer(item)}</BgBox>
								</li>
							))}
