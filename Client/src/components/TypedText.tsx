...
class TypedText extends React.Component<TypedTextProps, TypedTextState> {
		...
		let intervallId = setInterval(async () => {
			if (reverse) {
				...
				if (animation.chars.value.length - 1 === animation.chars.index) {
					...
					clearInterval(intervallId);
					if (
						["infinite", "infinite-reverse"].includes(animation.mode) ||
						(animation.mode === "stay" &&
							animation.words.value.length > 1 &&
							animation.words.index !== animation.words.value.length) ||
						(animation.mode === "reverse" &&
							animation.words.value.length > 1 &&
							!(
								animation.words.index === animation.words.value.length &&
								animation.words.blocked
							))
					) {
						setTimeout(() => {
				...
				if (
					animation.chars.value.length === 0 &&
					!animation.words.value[animation.words.index]
				) {
					clearInterval(intervallId);
					this.resetWords();
					return;
					...
					if (this.props.onWordChange)
						this.props.onWordChange(animation.words.value[animation.words.index]);
	...
	render() {
		let {
			animation: { displayedChars },
		} = this.state;
		const { fontSize, fontFamily = "'Roboto Mono', monospace" } = this.props;
		return (
			<span style={{ fontFamily }} className={`${fontSize} typed-text`}>
				{!this.state.rendered && this.props.initialHeight && (
					<>
						<span className="invisible">placeholder</span>
					</>
				)}
				{displayedChars.map((c, index) => (
