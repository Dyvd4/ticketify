...
function List(props: ListProps) {
	...
	const infiniteLoadingQuery = useInfiniteQuery<InfiniteLoaderResult>(
	...
	const paginationQuery = useQuery<PagerResult>(
		[queryKey, queryParams, itemsPerPage, parseInt(page)],
		{
			route,
			queryParams: {
				...queryParams,
	...
	const paginationQueryCount = paginationQuery.data?.items // prettier-ignore
	...
	const handleDrawerApply = (type: TDrawer) => {
		const itemsToSet = type === "filter" ? filterItems : sortItems;

		// MayBe: provide a utility for this
		if (
			(type === "filter" && currentUserSettings.allowFilterItemsByUrl) ||
			(type === "orderBy" && currentUserSettings.allowSortItemsByUrl)
		) {
			setUrlParam(`${type}-${props.id}`, itemsToSet);
		}
		if (
			(type === "filter" && currentUserSettings.allowFilterItemsByLocalStorage) ||
	...
	const headerIsVisible =
		header?.title || header?.showCount || useSearch || useSort || useFilter || props.onAdd;
