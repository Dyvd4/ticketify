...
export const useTicketIsPinned = (
	ticketId: number
): [ticketIsPinned: boolean, isLoading: boolean] => {
	const [pinnedTickets, isLoading] = usePinnedTickets();
	const ticketIsPinned =
		pinnedTickets && pinnedTickets.find((pinnedTicket) => pinnedTicket.id === +ticketId!);
	return [ticketIsPinned, isLoading];
};

export const usePinnedTickets = (): [pinnedTickets: any, isLoading: boolean] => {
	const { isLoading, ...query } = useQuery(["pinnedTickets"], () =>
		fetchEntity({
...
export const mutatePinnedTicket = (ticketId: string, ticketIsPinned: boolean) => {
	return ticketIsPinned ? removePinnedTicket(+ticketId!) : addPinnedTicket(+ticketId!);
};
